...
u08   ubLCD_len = 0;                            //--- LCD or USART Out Length
...
//---------------------------------------------------------------------------------------------
//    LCD Display Working Registers & Memort  4 Line 20 Char Display
//    These Memory values are defined in "char_lcd_dvr.c & .h"
//---------------------------------------------------------------------------------------------
//  u16   uwLCD_IP;                                   //--- LCD In pointer
//  u16   uwLCD_OP;                                   //--- LCD Out pointer
//  u16   uwLCDcnt;                                   //--- LCD FIFO Counter
//  union WB unLCDfifo [256];                         //--- ASCII / CMD FIFO for LCD Display
//
//  u08   ubBXD [6];                                  //--- Buffer with next sequence
//  union WB unWB;                                    //    Processing union
//

//--- Note: ubLCDflags defined in char_lcd_dvr.c & .h

void fnSysTick(void)                              //--- IRQ_- 1
{
    u16 uwErrCode = 0;                              //--- Generic Error Code
    ...
    switch (uwPacerPhase)
    {
        ...
        case 0x09:
            ...
            //-----------------------------------------------------------------------------------------
            //    If you get here something has gone horribly wrong
            //-----------------------------------------------------------------------------------------
        default:
    ...
    #ifdef USART2_FLAG
      //--- Test & Bump UART2 Tx Delay Count
      if (uwUSART2_DelCnt)                            //--- Is Xmit Timer Active?
      {
        uwUSART2_DelCnt--;                            //--- Yes so decrement Counter
        if (!uwUSART2_DelCnt)                         //--- Did we reach TX period time?
        {
          uwUSART2_DelCnt = XMIT_2_DELAY;             //--- Refresh the Period Count
          fnUSART2_XmitSvc();                        //--- Call the Xmit Service Routine
        }
      }
    #endif
    ...
    #ifdef USART1_FLAG
      //--- Test & Bump UART2 Tx Delay Count
      if (uwUSART1_DelCnt)                            //--- Is Xmit Timer Active?
      {
        uwUSART1_DelCnt--;                            //--- Yes so decrement Counter
        if (!uwUSART1_DelCnt)                         //--- Did we reach TX period time?
        {
          uwUSART1_DelCnt = XMIT_1_DELAY;             //--- Refresh the Period Count
