...
//===========================================================================================//
  ...
  #define uuIRQm8_IRQn               -8   //--- Undefined
  #define uuIRQm7_IRQn               -7   //--- Undefined
  #define uuIRQm6_IRQn               -6   //--- Undefined
  ...
  #define uuIRQi29_IRQn              29   //--- Undefined

  #define uuIRQi30_IRQn              30   //--- Undefined
  ...
  #define uuIRQi42_IRQn              42   //--- Undefined
  #define uuIRQi43_IRQn              43   //--- Undefined
  #define uuIRQi44_IRQn              44   //--- Undefined
  #define uuIRQi45_IRQn              45   //--- Undefined
  #define uuIRQi46_IRQn              46   //--- Undefined
  #define uuIRQi47_IRQn              47   //--- Undefined
  #define uuIRQi48_IRQn              48   //--- Undefined
  #define SDMMC1_IRQn                49   //--- SDMMC1 global Interrupt

  #define uuIRQi50_IRQn              50   //--- Undefined
  #define SPI3_IRQn                  51   //--- SPI3 global Interrupt
  #define uuIRQi52_IRQn              52   //--- Undefined
  #define uuIRQi53_IRQn              53   //--- Undefined
  ...
  #define uuIRQi61_IRQn              61   //--- Undefined
  #define uuIRQi62_IRQn              62   //--- Undefined
  #define uuIRQi63_IRQn              63   //--- Undefined
  ...
  extern u64 uxSysTicker;
  extern u32 ulDownCount;
  ...
  void  fnSetPendInt (u16 uwIntNum);
  void  fnClrPendInt (u16 uwIntNum);
  ...
  __STATIC_INLINE void NVIC_SetPriorityGrouping(u32 PriorityGroup)
  {
    u32 reg_value;
    u32 PriorityGroupTmp = (PriorityGroup & (u32)0x07UL);  //--- only values 0..7 are used

    reg_value  =  SCB_AIRCR;                              //--- read old register configuration
    reg_value &= ~((u32)(SCB_AIRCR_VECTKEY_Msk | SCB_AIRCR_PRIGROUP_Msk)); //--- clear bits to change
    reg_value  =  (reg_value                                   |
                  ((u32)0x05FAUL << SCB_AIRCR_VECTKEY_Pos) |
  ...
  __STATIC_INLINE u32 NVIC_GetPriorityGrouping(void)
  {
    return ((u32)((SCB_AIRCR & SCB_AIRCR_PRIGROUP_Msk) >> SCB_AIRCR_PRIGROUP_Pos));
  }
  ...
  __STATIC_INLINE u32 NVIC_GetEnableIRQ(u32 IRQn)
  {
    if ((s32)(IRQn) >= 0)
    {
      return((u32)(((NVIC->ISER[(((u32)IRQn) >> 5UL)] & (1UL << (((u32)IRQn) & 0x1FUL))) != 0UL) ? 1UL : 0UL));
    }
    else
    {
      return(0U);
  ...
  __STATIC_INLINE void NVIC_SetPriority(u32 IRQn, u32 priority)
  {
    if ((s32)(IRQn) >= 0)
    {
      NVIC->IP[((u32)IRQn)]               = (u08)((priority << (8U - __NVIC_PRIO_BITS)) & (u32)0xFFUL);
    }
    else
    {
      SCB->SHP[(((u32)IRQn) & 0xFUL)-4UL] = (u08)((priority << (8U - __NVIC_PRIO_BITS)) & (u32)0xFFUL);
    }
  }
