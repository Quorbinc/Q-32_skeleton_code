...
__attribute__ ((section(".isr_vector"))) void (* const g_pfnVectors[]) (void) =
{
  ...
  fnNull_IRQ,            // 92 - Blank                                 0x000001B0
  fnNull_IRQ,            // 93 - Blank                                 0x000001B4
  fnNull_IRQ,            // 94 - Blank                                 0x000001B8
  fnNull_IRQ,            // 95 - Blank                                 0x000001BC
  fnNull_IRQ,            // 96 - Blank                                 0x000001C0
  fnNull_IRQ,            // 97 - Blank                                 0x000001C4
  fnNull_IRQ,            // 98 - Blank                                 0x000001C8
  fnNull_IRQ,            // 99 - Blank                                 0x000001CC
};
...
//---------------------------------------------------------------------------------------------
//  Function Name  : SVCIRQ Software Generated Interrupt
//
//  Only Value #5 is used - The user may use the other Values available
//---------------------------------------------------------------------------------------------
...
//  Typically this function runs at 0.1 mSec This is a VERY High priority
//---------------------------------------------------------------------------------------------
u64 uxSysTicker;                                //--- Timer 64 Bit
u32 ulDownCount;                                //--- Counter 32 Bit
...
//---------------------------------------------------------------------------------------------
//    Enable Individual Interrupts  (0 to 99)
//---------------------------------------------------------------------------------------------
void  fnEnaInt (u16 uwIntNum)
{
  GID;                                            //--- Kill Other Interrupts
  if (uwIntNum >= 0) NVIC_EnableIRQ(uwIntNum);
  GIE;                                            //--- Enable Other Interrupts
}

//---------------------------------------------------------------------------------------------
//    Disable Individual Interrupt (0 to 99)
//---------------------------------------------------------------------------------------------
void  fnDisInt (u16 uwIntNum)
{
  GID;                                            //--- Kill Other Interrupts
  if (uwIntNum >= 0) NVIC_DisableIRQ(uwIntNum);
  GIE;                                            //--- Enable Other Interrupts
}
...
{
  GID;                                            //--- Kill Other Interrupts
  if (uwIntNum >= 0) NVIC_SetPendingIRQ(uwIntNum);
  GIE;                                            //--- Enable Other Interrupts
}
...
{
  GID;                                            //--- Kill Other Interrupts
  if (uwIntNum >= 0) NVIC_ClearPendingIRQ(uwIntNum);
  GIE;                                            //--- Enable Other Interrupts
}
...
{
  if (uwIntNum >= 0) NVIC_SetPriority(uwIntNum, ubPri);
}
